...
export default function Hero() {
  return (
    <section id="hero" style={{ position: 'relative', minHeight: '100vh', display: 'flex', alignItems: 'center', overflow: 'hidden', background: '#04040f' }}>
      ...
      {/* ── Content ── */}
      <div style={{ position: 'relative', zIndex: 10, width: '100%', maxWidth: '1100px', margin: '0 auto', padding: '0 1rem' }}>
        <motion.div variants={container} initial="hidden" animate="show" style={{ maxWidth: '620px' }}>
          ...
          <motion.p variants={item} style={{
            ...
            Full-stack developer obsessed with immersive interfaces,
            3D web experiences, and code that performs as good as it looks.
          </motion.p>

          {/* Role pills */}
          <motion.div variants={item} style={{ display: 'flex', flexWrap: 'wrap', gap: '0.75rem', marginBottom: '3rem' }}>
            {roles.map(role => (
              <span key={role} style={{
                fontFamily: 'var(--font-body)',
                fontSize: '0.78rem',
          ...
          <motion.div variants={item} style={{ display: 'flex', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
            <button
              ...
              style={{
                ...
                cursor: 'none',
                transition: 'all 0.3s',
                boxShadow: '0 0 28px rgba(124,58,237,0.5), inset 0 1px 0 rgba(255,255,255,0.1)',
              }}
              onMouseEnter={e => {
                const el = e.currentTarget as HTMLElement
                el.style.transform = 'translateY(-2px)'
                el.style.boxShadow = '0 0 45px rgba(124,58,237,0.75), inset 0 1px 0 rgba(255,255,255,0.15)'
              }}
              onMouseLeave={e => {
                const el = e.currentTarget as HTMLElement
                el.style.transform = ''
                el.style.boxShadow = '0 0 28px rgba(124,58,237,0.5), inset 0 1px 0 rgba(255,255,255,0.1)'
              }}
            >
              View My Work
            </button>
