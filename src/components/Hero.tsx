  ...
  return (
    <section id="hero" style={{ position: 'relative', minHeight: '100vh', display: 'flex', alignItems: 'center', overflow: 'hidden', background: '#04040f' }}>
      ...
      <div style={{ position: 'absolute', inset: 0, zIndex: 0 }}>
        <Canvas camera={{ position: [0, 0, 6], fov: 50 }} gl={{ antialias: true, alpha: true }} dpr={[1, 2]}>
          ...
          <Suspense fallback={null}>
            ...
            <Stars radius={60} depth={50} count={isMobile ? 1500 : 3000} factor={3} saturation={0.5} fade speed={0.4} />
            <Orb /><Ring /><RingOuter />
            {!isMobile && <Particles />}
          </Suspense>
          <CameraRig />
      ...
      <div style={{
        ...
        padding: isMobile ? '0 1.5rem' : isTablet ? '0 3rem' : '0 10rem',
      }}>
        <motion.div variants={container} initial="hidden" animate="show"
          style={{ maxWidth: isMobile ? '100%' : '620px' }}>

          {/* Name */}
          <motion.h1 variants={item} style={{
            fontFamily: 'var(--font-display)', fontWeight: 700,
            fontSize: isMobile ? 'clamp(3rem, 14vw, 4rem)' : 'clamp(3.8rem, 7vw, 7rem)',
          ...
          <motion.p variants={item} style={{
            fontFamily: 'var(--font-body)', fontSize: isMobile ? '0.9rem' : '1.05rem',
            lineHeight: 1.75, color: '#94a3b8', fontWeight: 300,
          ...
          <motion.div variants={item} style={{ display: 'flex', flexWrap: 'wrap', gap: '0.6rem', marginBottom: '3rem' }}>
            {roles.map(role => (
              <span key={role} style={{
                fontFamily: 'var(--font-body)', fontSize: isMobile ? '0.7rem' : '0.78rem',
                fontWeight: 500, padding: '0.45rem 1rem', borderRadius: '999px',
          ...
          <motion.div variants={item} style={{ display: 'flex', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
            ...
            <button onClick={() => document.querySelector('#contact')?.scrollIntoView({ behavior: 'smooth' })}
              style={{ padding: isMobile ? '0.8rem 1.6rem' : '0.9rem 2.2rem', fontSize: '0.82rem', fontFamily: 'var(--font-body)', fontWeight: 500, letterSpacing: '0.06em', textTransform: 'uppercase', color: '#a78bfa', background: 'transparent', border: '1px solid rgba(167,139,250,0.35)', borderRadius: '6px', cursor: 'none', transition: 'all 0.3s' }}
