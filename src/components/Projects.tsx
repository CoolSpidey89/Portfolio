  ...
  return (
    <div style={{ position: 'sticky', top: `${80 + index * 24}px`, zIndex: index + 1 }}>
      <motion.div
        ...
        style={{ borderRadius: '20px', overflow: 'hidden', border: `1px solid ${project.color}33`, background: project.gradient, boxShadow: `0 24px 80px rgba(0,0,0,0.5), 0 0 0 1px ${project.color}22`, marginBottom: '1.5rem' }}
      >
        {/* Video preview */}
          ...
          {project.video ? (
            <video src={project.video} autoPlay muted loop playsInline
              ...
              onLoadStart={e => {
                ...
                const observer = new IntersectionObserver(([entry]) => { if (entry.isIntersecting) v.play() }, { threshold: 0.3 })
                observer.observe(v)
              }}
            />
          ) : (
        ...
        {/* Info */}
        <div className="project-card-body" style={{ padding: '2rem 2.5rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '2rem', flexWrap: 'wrap' }}>
          <div style={{ flex: 1, minWidth: '280px' }}>
            <div style={{ display: 'flex', gap: '0.6rem', flexWrap: 'wrap', marginBottom: '1rem' }}>
              {project.tags.map(tag => (
                <span key={tag} style={{ padding: '0.3rem 0.85rem', fontFamily: 'var(--font-body)', fontSize: '0.72rem', fontWeight: 600, letterSpacing: '0.06em', textTransform: 'uppercase', color: '#e2e8f0', background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: '6px' }}>
                  {tag}
                </span>
              ))}
            </div>
            <h3 style={{ fontFamily: 'var(--font-display)', fontWeight: 600, fontSize: '1.8rem', color: '#fff', marginBottom: '0.25rem', lineHeight: 1.1 }}>
              {project.title}
              <span style={{ fontFamily: 'var(--font-body)', fontWeight: 300, fontSize: '1rem', color: '#94a3b8', marginLeft: '0.75rem' }}>— {project.subtitle}</span>
            </h3>
            <p style={{ fontFamily: 'var(--font-body)', fontSize: '0.9rem', color: '#64748b', lineHeight: 1.7, marginTop: '0.75rem', maxWidth: '520px' }}>{project.desc}</p>
          </div>

          <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center', flexShrink: 0, flexWrap: 'wrap' }}>
            <a href={project.code} target="_blank" rel="noreferrer"
              style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.7rem 1.4rem', fontFamily: 'var(--font-body)', fontSize: '0.82rem', fontWeight: 500, color: '#e2e8f0', background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: '8px', textDecoration: 'none', cursor: 'none', transition: 'all 0.25s' }}
            ...
            {project.demo ? (
              <a href={project.demo} target="_blank" rel="noreferrer"
                style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.7rem 1.4rem', fontFamily: 'var(--font-body)', fontSize: '0.82rem', fontWeight: 600, color: '#fff', background: `linear-gradient(135deg, ${project.color}, ${project.color}bb)`, border: 'none', borderRadius: '8px', textDecoration: 'none', cursor: 'none', transition: 'all 0.25s', boxShadow: `0 0 20px ${project.color}44` }}
              ...
              <span style={{ padding: '0.7rem 1.4rem', fontFamily: 'var(--font-body)', fontSize: '0.82rem', fontWeight: 500, color: '#334155', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '8px' }}>
                No Live Demo
              </span>
            )}
          </div>
...
export default function Projects() {
  ...
  return (
    <>
      <style>{`
        .project-card-body { padding: 2rem 2.5rem !important; flex-direction: row !important; }
        @media (max-width: 768px) {
          .project-card-body { padding: 1.25rem !important; flex-direction: column !important; }
          .project-card-body h3 { font-size: 1.3rem !important; }
          .project-card-body h3 span { display: block !important; margin-left: 0 !important; margin-top: 0.25rem !important; }
          #projects .projects-wrap { padding: 0 1.25rem !important; }
        }
      `}</style>
      ...
      <section id="projects" ref={containerRef} style={{ position: 'relative', padding: '2rem 0 4rem', background: '#04040f' }}>
        ...
        <div className="projects-wrap" style={{ maxWidth: '900px', margin: '0 auto', padding: '0 4rem' }}>
          <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7, ease: [0.22, 1, 0.36, 1] as const }} style={{ textAlign: 'center', marginBottom: '5rem' }}>
